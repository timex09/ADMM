...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  7 07:48:52 2020

@author: Leonard Schulte
"""
...
def compute(nodes, options, week, net_data, params):
   ...
   # for n in nodes:
       # c_P2P[n]={}
        #for t in time_steps:
           # c_P2P[n][t]=model.addVar(vtype="C", name="price_P2P" + str(n) + "_t" + str(t))
...
#%% EV CONSTRAINTS CONSTRAINTS
  ...
  #  for n in nodes:
    ...
    for n in nodes:
        for j in nodes:
            for t in time_steps:
                ...
                r["Leistung"][n][t] = power_P2P_sum["inj"][n][t]
    #for c in model.getConstrs():
    ...
    test=[r["pi"][3][2][t] for t in time_steps]
    print(test)


    Preise = [r["Leistung"][2][t] for t in time_steps]
    print(Preise)
