...
def opti(nodes, options, week, net_data, params, z, admm_params, Ubergabedaten, counter, lam):
    ...
    #  binary[device][n] = {}
    # for t in time_steps:
    # binary[device][n][t] = model.addVar(vtype= gp.GRB.BINARY, name="factor_binary_" + device + "_n" + str(z) + "_t" + str(t))
    ...
    for t in time_steps:
        ...
        # model.addConstr(ch_dom["BAT"][n][t] <= binary["BAT"][n][t]*100, name ="Binary1__bat" + str(z) + "_" + str(t))
        # model.addConstr(dch_dom["BAT"][n][t] <= (1-binary["BAT"][n][t])*100, name ="Binary2__bat" + str(z) + "_" + str(t))
    ...
    # for n in nodes:
    for t in time_steps:
        model.addConstr(power_P2P_sum["inj"][z][t] == sum(power_P2P["inj"][z][j][t] for j in nodes),
                        name="P2P" + str(z) + "_" + str(t))
        model.addConstr(power_P2P_sum["load"][z][t] == sum(power_P2P["load"][z][j][t] for j in nodes),
                        name="P2P" + str(z) + "_" + str(t))
    ...
    model.addConstr(
        operational_costs == sum(
            power_grid["load"][z][t] * params["eco"]["elec_price"] - power_grid["inj"][z][t] * params["eco"][
                "eeg_pv"] + (power_P2P["load"][z][j][t] * lam[j][t])
            + power_P2P_sum["inj"][z][t] * lam[z][t] + options["rho"] / 2 * (
                        ((power_P2P["inj"][z][j][t] - Ubergabedaten["P_P2P_load"][j][z][t]) ** 2) + (
                            (Ubergabedaten["P_P2P_inj"][j][z][t] - power_P2P["load"][z][j][t]) ** 2)) for j in nodes for
            t in time_steps))
