...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 20 09:22:06 2020
...
def create_kerbernet_bedburg(std_type="NAYY 4x185", p_load_mw=.002, q_load_mvar=0.,
    ...
    num_lines = [18, 25, 23, 25, 14, 8]
    len_lines = [0.010, 0.014, 0.023, 0.025, 0.014, 0.050]
...
net = create_kerbernet_bedburg(std_type="NAYY 4x185", p_load_mw=.002, q_load_mvar=0.,
                                       trafotype="0.63 MVA 10/0.4 kV", v_os=10.)
...
net_nodes["grid"] = net.bus.index.values
net_nodes["trafo"] = net.trafo['lv_bus'].values
net_nodes["load"] = net.load['bus'].values
#nodeInj = net.load['bus'].to_numpy()
net_nodes["bat"] = net.load['bus'].values

gridnodes = list(net_nodes["grid"])
...
for key in powerLine_max.keys():
    
    cur_tuple = key
    
    lines.append(tuple(cur_tuple))
